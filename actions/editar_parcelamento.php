...
if ($id <= 0 || $descricao === '' || $valor_total <= 0 || $valor_total > 1000000000 || $num_parcelas < 2 || $num_parcelas > 60 || $data_inicio === '') {
...
if (!mysqli_query($conexao, $sql_update)) {
    die("Erro ao atualizar parcelamento: " . mysqli_error($conexao));
}

mysqli_query($conexao, "DELETE FROM transacoes WHERE parcelamento_id = '$id' AND usuario_id = '$usuario_id'");

$valor_parcela = $valor_total / $num_parcelas;

for ($i = 1; $i <= $num_parcelas; $i++) {
    $data_parcela = date('Y-m-d', strtotime("+" . ($i - 1) . " month", strtotime($data_inicio)));
    ...
    $sql_transacao = "INSERT INTO transacoes (usuario_id, descricao, valor, categoria, data, parcelamento_id)
                      VALUES ('$usuario_id', '$desc_formatada', '$valor_negativo', '$categoria', '$data_parcela', '$id')";

    if (!mysqli_query($conexao, $sql_transacao)) {
        die("Erro ao recriar parcelas: " . mysqli_error($conexao));
    }
}

$_SESSION['message'] = "Parcelamento atualizado com sucesso!";
redirectToIndexWithMonthYear();
exit;
?>
